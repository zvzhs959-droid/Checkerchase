...
spawn(pos, vel, life, scale, colorHex = null, rotation = null) {
            ...
            if (!p) {
                index = Math.floor(window.flux.rand() * this.maxCount);
                p = this.data[index];
            }
            ...
            if (rotation) {
                ...
                // Random rotation
                p.rotation.set(window.flux.rand()*Math.PI, window.flux.rand()*Math.PI, window.flux.rand()*Math.PI);
                p.rotationSpeed.set(
                    (window.flux.rand() - 0.5) * 10,
                    (window.flux.rand() - 0.5) * 10,
                    (window.flux.rand() - 0.5) * 10
                );
            }
...
// Reuse _pVel
                _pVel.set(
                    (window.flux.rand() - 0.5) * spread * 5,
                    window.flux.rand() * 3 + 2,
                    (window.flux.rand() - 0.5) * spread * 5
                );
                const vel = _pVel;
                let life = 0.5 + window.flux.rand() * 0.5;
                let scale = 1.0;
                let pColor = color;
                rotation = null;
                ...
                // Specific Overrides
                if (type === 'DUST') {
vel.y = window.flux.rand() * 1;
                    life = 0.4;
                } 
                else if (type === 'DEBRIS') {
                    if (!pColor) pColor = window.flux.rand() > 0.5 ? 0x553311 : 0xaa8855;
                    scale = 0.8 + window.flux.rand() * 0.5;
                } 
                else if (type === 'SPARK') {
...
vel.set(
                        (window.flux.rand() - 0.5) * 4,
                        4 + window.flux.rand() * 3,
                        (window.flux.rand() - 0.5) * 4
                    );
                    life = 0.5;
                    scale = 1.0 + window.flux.rand() * 1.5;
                    if (!pColor) pColor = 0xffffff;
                } 
                else if (type === 'TRAIL') {
vel.set(
                        (window.flux.rand() - 0.5) * spread, 
                        window.flux.rand() * 0.5, 
                        (window.flux.rand() - 0.5) * spread
                    );
                    life = 0.6;
                    scale = 0.6 + window.flux.rand() * 0.6;
                    if (!pColor) pColor = 0xffffff;
                }
                    ...
                    life = 0.15;
                    scale = 0.5 + window.flux.rand() * 0.5;
                    if (!pColor) pColor = 0xffffff;
