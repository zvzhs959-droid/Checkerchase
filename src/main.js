...
// --- Shared Constants ---
        ...
        window.flux.rebuildControllers = function() {
            ...
            const controllers = [];
            // In lockstep multiplayer, ALL human-controlled slots use LockstepController (including local slot)
            if (net && net.lockstepEnabled && window.flux.LockstepController) {
                ...
                window.flux.localController.slot = localSlot;
            } else {
            ...
            for (let slot = 0; slot < 4; slot++) {
                ...
                if (net && net.lockstepEnabled && isRemote && window.flux.LockstepController) {
                    const c = new window.flux.LockstepController(players[slot], slot);
                    c.slot = slot;
                    controllers.push(c);
                } else if (isRemote && window.flux.NetController && !(net && net.lockstepEnabled)) {
        ...
        window.flux.startMatch = function(opts) {
            ...
            const seed = (typeof opts.seed === 'number') ? (opts.seed >>> 0) : ((net && typeof net.lockstepSeed === 'number') ? (net.lockstepSeed >>> 0) : 123456789);
            ...
            if (useLockstep && window.flux.Lockstep) {
                const playerCount = (typeof opts.playerCount === 'number') ? (opts.playerCount|0) : 2;
                // Reset lockstep to frame 0 every match start
                window.flux.lockstep = new window.flux.Lockstep({
                    slot: (net && typeof net.slot === 'number') ? net.slot : 0,
                    ...
                    onFrame: (frame, bitsBySlot, dtFixed) => {
                        ...
                        if (window.flux.localController && typeof window.flux.localController.setBits === 'function') {
                            window.flux.localController.setBits(bitsBySlot[window.flux.localSlot]>>>0);
                        }
                        if (window.flux.cpus) {
                            for (let i = 0; i < window.flux.cpus.length; i++) {
