...
function applySnapshot(snap) {
            ...
            for (let i = 0; i < players.length; i++) {
                ...
                if (controller && typeof controller.onSnapshot === 'function' && !isLocal) {
                    ...
                    // Always accept authoritative velocity + state + stats
...
const gs = window.flux.gameState;
            ...
            const __isHostAuth = (!window.flux.net || window.flux.net.slot === 0);
            // Shrink Logic
...
// (Duplicate shockwaveManager update removed)
            if (window.flux.powerups) window.flux.powerups.update(dt, (!window.flux.net || window.flux.net.slot === 0));
