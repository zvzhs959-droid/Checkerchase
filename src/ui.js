...
this.topBar.appendChild(this.stockContainer);
            // Pause/Menu Button (Top Right, not inside stock bar)
            this.menuBtn = document.createElement('button');
            this.menuBtn.className = 'crayon-btn crayon-btn--ink sb-icon-btn';
            ...
            this.bindTap(this.menuBtn, () => this.togglePauseMenu());
...
createCosmeticMesh(id) {
            ...
            if (id === 'crown_gold') {
                ...
                for(let i=0; i<numCurls; i++) {
                    ...
                    const r = 0.33;
                    m.position.set(Math.cos(angle)*r, 0.25+(window.flux.rand()-0.5)*0.06, Math.sin(angle)*r);
                    m.rotation.set(window.flux.rand()*3, window.flux.rand()*3, window.flux.rand()*3);
                    m.scale.setScalar(0.8 + window.flux.rand() * 0.4);
                    mesh.add(m);
                }
...
const joinBtn = document.createElement('button');
        ...
        createPauseOverlay() {
            ...
            this.pauseOverlay.className = 'sb-overlay sb-pause';
            this.pauseOverlay.style.zIndex = '165';
            this.pauseOverlay.style.display = 'none';

            this.pauseCanvas = document.createElement('div');
            this.pauseCanvas.className = 'sb-canvas-pad';

            this.pauseCanvas.innerHTML = `
                <div class="sketch-card card-blue" style="max-width:520px; margin:0 auto;">
                    <h1 class="sb-title" style="margin-bottom:10px;">Menu</h1>
                    <div style="display:flex; flex-direction:column; gap:12px;">
                        <button id="pause-resume" class="crayon-btn crayon-btn--green">RESUME</button>
                        <button id="pause-restart" class="crayon-btn crayon-btn--yellow">RESTART MATCH</button>
                        <button id="pause-exit" class="crayon-btn crayon-btn--red">EXIT MATCH</button>
                    </div>
                    <div style="margin-top:10px; font-family:'Space Mono', monospace; font-size:12px; opacity:0.65;">
                        (Host can restart for everyone.)
                    </div>
                </div>
            `;

            this.pauseOverlay.appendChild(this.pauseCanvas);
            this.container.appendChild(this.pauseOverlay);

            const resumeBtn = this.pauseCanvas.querySelector('#pause-resume');
            const restartBtn = this.pauseCanvas.querySelector('#pause-restart');
            const exitBtn = this.pauseCanvas.querySelector('#pause-exit');

            this.bindTap(resumeBtn, () => this.togglePauseMenu(false));
            this.bindTap(restartBtn, () => {
                if (window.flux.requestRestartMatch) window.flux.requestRestartMatch();
                this.togglePauseMenu(false);
            });
            this.bindTap(exitBtn, () => {
                if (window.flux.exitMatchToMenu) window.flux.exitMatchToMenu();
                this.togglePauseMenu(false);
            });

            // Tap background to resume
            this.bindTap(this.pauseOverlay, (e) => {
                if (e && e.target !== this.pauseOverlay) return;
                this.togglePauseMenu(false);
            });
        }

        togglePauseMenu(forceOpen) {
            const isHidden = this.pauseOverlay.style.display === 'none' || this.pauseOverlay.style.display === '';
        ...
        showLobby(isHost, roomId) {
            ...
            this.updateLobbyList([], [], (window.flux.net ? window.flux.net.slotOpen : null), isHost); // Clear initially
        }
        ...
        updateLobbyList(playersPresent, names, slotOpen, isHost) {
            this.lobbyList.innerHTML = '';
            const labels = ["P1 (HOST)", "P2", "P3", "P4"];
            const colors = ["#0088FF", "#FF3333", "#00CC00", "#FFAA00"];

            const openSlots = slotOpen || [true, true, true, true];
            ...
            for (let i = 0; i < 4; i++) {
                ...
                const isOpen = !!openSlots[i];

                let name = (names && names[i]) ? names[i] : "Waiting...";
                if (!isPresent && !isOpen) name = "CLOSED";

                row.style.opacity = isPresent ? '1' : (isOpen ? '0.6' : '0.45');
                row.style.borderColor = isPresent ? colors[i] : (isOpen ? '#ccc' : 'rgba(43,45,66,0.35)');
                ...
                row.innerHTML = `
                    ...
                    <div style="display:flex; align-items:center; gap:10px;">
                        ${hostMode && i > 0 && !isPresent ? `<button class="crayon-btn crayon-btn--ink sb-icon-btn" data-slot-toggle="${i}">${isOpen ? 'OPEN' : 'CLOSED'}</button>` : ''}
                        <div>${isPresent ? 'READY' : (isOpen ? '...' : '×')}</div>
                    </div>
                `;

                // Host can open/close empty slots (prevents joins and disables CPU spawn)
                if (hostMode && i > 0 && !isPresent) {
                    const tbtn = row.querySelector('[data-slot-toggle]');
                    if (tbtn) {
                        this.bindTap(tbtn, () => {
                            if (window.flux.audio) window.flux.audio.playSFX('UI_CLICK');
                            if (window.flux.net && window.flux.net.toggleSlotOpen) {
                                window.flux.net.toggleSlotOpen(i);
                            }
                        });
                    }
                }
        ...
        hideMainMenu() {
            ...
            if (this.menuBtn) this.menuBtn.style.display = 'block';
        }
    }
