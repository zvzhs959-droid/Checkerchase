...
class AIController {
        constructor(playerEntity, target) {
            ...
            // AI Parameters
            this.reactionTime = 0.1 + Math.random() * 0.2; // Seconds to react
            ...
            // Targeting
            this.retargetTimer = Math.random() * 2.0; // Stagger initial scans

            // Personality
            this.personality = {
                aggression: 0.4 + Math.random() * 0.6, // 0.4 - 1.0
                greed: 0.3 + Math.random() * 0.7,      // Powerup hunger
                fear: Math.random() * 0.5              // Tendency to flee
            };
...
update(dt) {
            ...
            if (!this.target || this.target.lives <= 0 || this.retargetTimer <= 0) {
                ...
                this.retargetTimer = 1.0 + Math.random() * 2.0;
            }
            ...
            if (this.reactionTimer <= 0) {
                this.reactionTimer = 0.25 + (Math.random() * 0.2); // Slower reaction (Human-like)
                this.evaluateGoal();
                this.decideMove();
                this.decideCombat();
            }
