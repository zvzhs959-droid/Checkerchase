...
(function() {
    ...
    const _waveTexture = (function() {
        ...
        for (let i = 0; i < 120; i++) {
            const x = window.flux.rand() * 512;
            const y = window.flux.rand() * 256;
            const w = 50 + window.flux.rand() * 150; // Long streaks
            const h = 2 + window.flux.rand() * 6;
            
            ctx.fillStyle = `rgba(255, 255, 255, ${window.flux.rand() * 0.4})`;
...
class Shockwave {
        ...
        update(dt) {
            ...
            this.mesh.rotation.z = (window.flux.rand() - 0.5) * 0.08;
        ...
        processTileHit(tile) {
            // Multiplayer: host-authoritative tile state
    ...
    class ShockwaveManager {
        ...
        spawn(owner, type) {
            ...
            // Host: emit a reliable shockwave-spawn event so clients can render distortion/trails consistently
if (window.flux && window.flux.net && window.flux.net.slot === 0 && typeof window.flux.queueNetEvent === 'function') {
    try {
        const fwd = owner.getForward ? owner.getForward() : { x: 0, z: 1 };
        const dx = (Math.abs(fwd.x) > Math.abs(fwd.z)) ? Math.sign(fwd.x || 1) : 0;
        const dz = (Math.abs(fwd.x) > Math.abs(fwd.z)) ? 0 : Math.sign(fwd.z || 1);
        const gx = Math.round(owner.position.x);
        const gz = Math.round(owner.position.z);
        const slot = (window.flux._players && window.flux._players.indexOf(owner) >= 0) ? window.flux._players.indexOf(owner) : 0;
        window.flux.queueNetEvent({
            k: 'SW',
            owner: slot,
            type: type,
            x: gx,
            ...
            dx: dx,
            dz: dz,
            crystal: !!(owner.activePowerup && owner.activePowerup.type === 'CRYSTAL')
        });
    } catch (e) {
        // no-op
    }
...
spawnFromNet(ev) {
    ...
    // Fake owner so Shockwave.init() can compute direction + optional crystal speed
    const owner = {
        position: new THREE.Vector3(ev.x || 0, 0.27, ev.z || 0),
        activePowerup: (ev.crystal ? { type: 'CRYSTAL' } : null),
        getForward: function() {
            return { x: ev.dx || 0, z: ev.dz || 1 };
        }
    };

    // Spawn both wave layers (visual-only tile logic is already host-guarded inside processTileHit)
    const w1 = this.getWave();
    w1.init(owner, ev.type, { geometry: _vGeo });
    this.waves.push(w1);

    const w2 = this.getWave();
    w2.init(owner, ev.type, { geometry: _geoWake, speedMult: 0.55 });
    this.waves.push(w2);

    // Audio/Shake locally for feedback (client-safe)
    if (window.flux && window.flux.renderer) window.flux.renderer.addShake(0.15);
    if (window.flux && window.flux.audio) window.flux.audio.playSFX('SHOCKWAVE');
}
