...
update(dt, input) {
// Multiplayer client: remote players are puppets (host-authoritative).
// Do NOT run gameplay/physics/state transitions locally for NetController-driven entities,
// otherwise FALL/DEATH/attacks can diverge and desync visuals/stocks.
const __isNetClientPuppet = (window.flux && window.flux.net && window.flux.net.slot !== 0 && input && input.isNetController);
if (__isNetClientPuppet) {
    // Advance animation timer for smooth visuals between snapshots
    this.stateTimer = (this.stateTimer || 0) + dt;

    // Keep mesh aligned with authoritative position
    if (this.mesh) this.mesh.position.copy(this.position);

    // Sync Shadow
    if (this.shadow) {
        this.shadow.position.set(this.position.x, 0.02, this.position.z);
        const height = Math.max(0, this.position.y - 0.27);
        const sScale = Math.max(0, 1.0 - height * 0.8);
        this.shadow.scale.setScalar(sScale);
        this.shadow.visible = this.mesh && this.mesh.visible && this.state !== 'DEAD';
    }

    // Visuals (invuln flicker)
    if (this.invulnTimer > 0 && this.mesh) {
        this.mesh.visible = Math.floor(window.performance.now() / 80) % 2 === 0;
    } else if (this.mesh) {
        this.mesh.visible = true;
    }

    // Run visuals/cosmetics/powerup visuals only
    this.updateVisuals();
...
if (this.state === 'IDLE') {
        ...
        updateEmissive() {
            if (this.state === 'CHARGING') {
                ...
                this.mesh.position.x += (window.flux.rand() - 0.5) * 0.05; // Shake
            }
...
performAttackEffect(data) {
            ...
            if (this.attack.type.startsWith('SHOCKWAVE')) {
                ...
                window.flux.audio.playSFX('SHOCKWAVE');
                        ...
                        const __isNetClient = (window.flux && window.flux.net && window.flux.net.slot !== 0);
            if (!__isNetClient) {
...
const targetVel = _inputVec.multiplyScalar(this.MAX_SPEED);
                 ...
                 if (this.dustTimer <= 0) {
                     ...
                     if (this.footR.position.y < this.footL.position.y) {
                         ...
                         targetFoot = window.flux.rand() > 0.5 ? this.footL : this.footR;
                     }
...
handleDash(dt) {
            ...
            if (this.stateTimer % 0.05 < dt) { // Throttle emission
                ...
                _pVec1.x += (window.flux.rand() - 0.5) * 0.5;
                _pVec1.z += (window.flux.rand() - 0.5) * 0.5;
...
const dir = _pVec1.copy(this.position).sub(attacker.position).normalize();
            ...
            if (dir.lengthSq() === 0) dir.set(window.flux.rand()-0.5, 0, window.flux.rand()-0.5).normalize();
...
// (Cleaned up comments)
onDeath() {
            // Multiplayer: only host should decrement lives / trigger match logic
            if (window.flux && window.flux.net && window.flux.net.slot !== 0) {
                return;
            }
