...
// --- Stone Texture (Granite + Diamond Pattern) ---
    function createStoneDiamondTexture() {
        ...
        // Heavy Noise
        for(let i=0; i<400; i++) {
            ctx.fillStyle = window.flux.rand() > 0.5 ? '#50505a' : '#8c8c96';
            const x = window.flux.rand() * 64;
            const y = window.flux.rand() * 64;
            const s = 1 + window.flux.rand() * 3;
        ...
        for(let i=0; i<3; i++) {
            ctx.beginPath();
            let cx = window.flux.rand() * 64;
            let cy = window.flux.rand() * 64;
            ctx.moveTo(cx, cy);
            for(let j=0; j<3; j++) {
                cx += (window.flux.rand() - 0.5) * 20;
                cy += (window.flux.rand() - 0.5) * 20;
                ctx.lineTo(cx, cy);
            }
            ctx.stroke();
        }
...
if (this.state === STATES.FALLING) {
        ...
        update(dt) {
            ...
            switch(this.state) {
                case STATES.CRACKED:
                    const shakeAmt = 0.05;
                    this.mesh.position.x = this.gridX + (window.flux.rand() - 0.5) * shakeAmt;
                    this.mesh.position.z = this.gridZ + (window.flux.rand() - 0.5) * shakeAmt;
                    break;
